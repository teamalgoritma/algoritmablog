...
date: '2020-04-13'
slug: time-efficiency-and-accuracy-improvement-using-pca
categories:
  - R
tags: 
  - Machine Learning
...
This article focus on the application of Principal Component Analysis (PCA) on classification analysis and the impact on the time efficiency and accuracy improvement.

# About Dimensionality Reduction

If you are familiar enough with data, sometimes you are faced with too many predictor variables that make the computation so heavy. Let us say, you are challenged to predict employee in your company will resign or not while the variables are the level of satisfaction on work, number of project, average monthly hours, time spend at the company, etc. You are facing so many predictor that took so long for training your model. One way to speed up your training process is by dimensionality reduction. Reducing the dimension of the variable can make the computation less heavy. 
...
```{r echo = F}
knitr::include_graphics('/img/pca_use_case/tictoc_pca.PNG')
```
...
Now, we want to compare the result of model that use PCA in the preprocessing process with the model using the same preprocessing, but without PCA. Here let us make the recipe first.
...
```{r echo = F}
knitr::include_graphics('/img/pca_use_case/tictoc_tnp_pca.PNG')
```
...
```{r echo = F}
model_cancer_pca <- readRDS("rf_cancer.rds")
```

<center>

```{r echo = F}
knitr::include_graphics('/img/pca_use_case/cancer+pca.PNG')
```
...
```{r echo = F}
model_cancer <- readRDS("rf_cancer2.rds")
```

<center>

```{r echo = F}
knitr::include_graphics('/img/pca_use_case/cancer+tnp+pca.PNG')
```
